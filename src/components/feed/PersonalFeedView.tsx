...
/**
 * The content of one feed category (rows, empty states, onboarding). The
 * page owns the category state and the layout around it.
 */
export const FeedCategoryContent = ({ category }: { category: FeedCategory }) => {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const { user } = useUser();
  const { feedItems, isLoading, markAsRead, trackEngagement, userArchetype, hoursSinceLastGrowth } = usePersonalFeed(category);
  ...
  return (
    <TabContent
      ...
      onTrackEngagement={(id) => trackEngagement.mutate(id)}
...
/** Tabs + content in one block (kept for any embedding that wants its own tabs). */
...
const ForYouTab = ({ feedItems, navigate, t, userId, onMarkRead, onTrackEngagement, userArchetype, hoursSinceLastGrowth }: any) => {
  ...
  return (
    <div>
      ...
      <p className="py-4 text-center text-[12px] text-muted-foreground">{t('feed.all_seen', 'You have seen all the updates.')}</p>
    </div>
  );
};
  ...
  if (!hasGrowthPath) {
    return (
      <div className="py-8 text-center">
          <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full border border-[hsl(var(--xs-line))] text-muted-foreground">
...
const OpportunityCard = ({ job, isMatch, t, navigate }: { job: any; isMatch: boolean; t: any; navigate: any }) => {
  ...
  return (
    <button type="button" onClick={() => navigate('/jobs')} className="block w-full border-b border-[hsl(var(--xs-line))] py-4 text-left last:border-b-0 hover:bg-[hsl(var(--xs-page))]/60">
        <div className="flex items-start justify-between">
          ...
          {isMatch && (
            <span className="ml-2 shrink-0 rounded-md border border-primary/40 px-2 py-0.5 font-mono text-[11px] text-primary">
