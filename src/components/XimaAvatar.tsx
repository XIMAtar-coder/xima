...
const XimaAvatar: React.FC<XimaAvatarProps> = ({ 
  ...
  return (
    <div className={`flex flex-col items-center ${showDetails ? 'space-y-4' : ''}`}>
      <div className={`${sizeClasses[size]} rounded-full overflow-hidden border-4 border-[#4171d6] bg-white flex items-center justify-center`}>
        <img 
      ...
      {showDetails && (
        <div className="text-center">
          <h3 className="text-lg font-medium text-gray-800">{avatar.animal}</h3>
            ...
            {avatar.features.map((feature, index) => (
              <div key={index} className="text-sm">
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-[#4171d6]">{feature.strength}/10</span>
                </div>
