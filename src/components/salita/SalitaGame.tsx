 ...
 *
 * The same game runs as its own page and as the last chapter of the
 * questionnaire; only the way in and the way out change, so the trial a
 * person meets is the same in both places, and the same for every field.
 */
...
interface Props {
  /** Leaving from the intro: back to the previous page, or the previous question. */
  onBack: () => void;
  /** The trial is over (solved, stopped or skipped): the summary is what was observed. */
  onDone: (summary: SalitaSummary | null) => void;
  /** Inside the questionnaire the chapter can be skipped like the other breaks. */
  onSkip?: () => void;
  /** Heading level: the page owns an h1, the questionnaire does not. */
  as?: 'h1' | 'h2';
  ...
  if (phase === 'intro') {
    return (
      <div>
        ...
        <Heading className="mt-2 text-[26px] font-semibold leading-tight tracking-[-0.8px] text-foreground sm:text-[32px]">{t('salita.intro_title')}</Heading>
  ...
  if (phase === 'debrief') {
    return (
      <div>
        ...
        <Heading className="mt-2 text-[24px] font-semibold leading-tight tracking-[-0.7px] text-foreground sm:text-[30px]">{t('salita.debrief_title')}</Heading>
      ...
      >
        {board.tiles.map((tl, i) => {
          ...
          const fits = [1, 2, 4, 8].every((bit, di) => {
            ...
            const nHas = inside && (board.tiles[nr * board.size + nc] & opp) !== 0;
            return has === nHas;
          });
