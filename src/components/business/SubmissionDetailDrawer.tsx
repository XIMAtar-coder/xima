...
export function SubmissionDetailDrawer({
  ...
  useEffect(() => {
    async function autoGenerateL2Signals() {
      ...
      // Skip if signals already exist in local state
      ...
      console.log('[L2 Signals] Starting AI generation for submission:', submission.submissionId);
      
      try {
        // Call edge function for AI-powered analysis
        const l2Signals = await computeLevel2SignalsAsync(submission.submissionId);
        ...
        console.log('[L2 Signals] Generation complete, isGeneratingLevel2 set to false');
      }
    }
    
    autoGenerateL2Signals();
  }, [open, submission?.submissionId, submission?.submissionStatus, submission?.submittedPayload, currentChallengeLevel, localLevel2Signals, onSignalsGenerated, t]);

  // Retry L2 signals generation
  const handleRetryL2Signals = async () => {
    if (!submission?.submissionId) return;
    autoGeneratedRef.current = null; // Clear to allow retry
    setLocalLevel2Signals(null);
    setLevel2Error(null);
  };
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-xl overflow-y-auto">
        {!submission ? (
          ...
          {/* Level 2 Loading State - Only show when actively generating */}
          {currentChallengeLevel === 2 && isGeneratingLevel2 && (
          ...
          {currentChallengeLevel === 2 && !localLevel2Signals && !isGeneratingLevel2 && level2Error && (
          ...
          {currentChallengeLevel === 2 && !localLevel2Signals && !isGeneratingLevel2 && !level2Error && submission.submissionStatus === 'submitted' && (
            <Card className="border-dashed">
              <CardContent className="py-6 text-center">
