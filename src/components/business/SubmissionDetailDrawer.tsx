...
import { computeLevel2Signals, computeLevel2SignalsAsync, Level2SignalsPayload } from '@/lib/signals/computeLevel2Signals';
...
export function SubmissionDetailDrawer({
  ...
  const signalsFromDb = useMemo((): Level2SignalsPayload | null => {
    if (currentChallengeLevel !== 2) return null;
    if (!submission?.signalsPayload) return null;
    const existingL2 = submission.signalsPayload as unknown as Level2SignalsPayload;
    return existingL2?.hardSkillClarity ? existingL2 : null;
  }, [submission?.signalsPayload, currentChallengeLevel]);

  // Effective L2 signals: local state takes precedence over DB
  const effectiveLevel2Signals = localLevel2Signals ?? signalsFromDb;

  // RESET state ONLY when submissionKey changes (new submission opened)
  useEffect(() => {
    if (!open) {
      // Drawer closed - reset everything
    ...
    // Only reset local L2 state when opening a DIFFERENT submission
    if (submissionKey && lastOpenedKeyRef.current !== submissionKey) {
      lastOpenedKeyRef.current = submissionKey;
      // Initialize from DB signals if available
      setLocalLevel2Signals(signalsFromDb);
      setLevel2Error(null);
      setIsGeneratingLevel2(false);
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-xl overflow-y-auto">
        {!submission ? (
          ...
          {currentChallengeLevel === 2 && isGeneratingLevel2 && !effectiveLevel2Signals && (
            <Card className="border-dashed">
              <CardContent className="py-6 text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1">Analyzing with XIMA AI...</p>
              </CardContent>
            </Card>
          )}
          
          {/* Level 2 Error State - Show retry button */}
          {currentChallengeLevel === 2 && !effectiveLevel2Signals && !isGeneratingLevel2 && level2Error && (
            <Card className="border-destructive border-dashed">
              <CardContent className="py-6 text-center">
                <AlertTriangle className="h-8 w-8 mx-auto mb-2 text-destructive" />
                <p className="text-destructive mb-2">AI interpretation failed</p>
                <p className="text-xs text-muted-foreground mb-3">{level2Error}</p>
          ...
          {/* Level 2 Generate Button - Only show if no signals, not generating, and no error */}
