 ...
 * 
 * This file defines the complete XIMAtar profile taxonomy used consistently across:
 * - Company Profile generator (edge function)
...
export interface XimatarProfile {
  ...
  // Pillar vector for distance calculations (0-100 scale)
  ...
  // Seniority fit (1-5 scale: 1=junior, 5=c-level)
  seniorityRange: {
    min: number;
    max: number;
  };
  
  // Ideal roles for this profile
  idealRoles: string[];
}

/**
 * The 12 canonical XIMAtar profiles with full taxonomy data
 */
...
/**
 * Get array of all XIMAtar IDs
 */
export const XIMATAR_IDS = Object.keys(XIMATAR_PROFILES);

/**
 * Get just the pillar vectors (for compatibility with existing code)
 */
...
/**
 * Default neutral pillar vector (used when no company profile exists)
 */
...
/**
 * Compute pillar vector from text analysis (for edge function compatibility)
 */
...
/**
 * Rank all XIMAtars by distance to a target pillar vector
...
/**
 * Get top N recommended XIMAtars for a pillar vector
 */
...
/**
 * Match XIMAtars by keyword overlap with company/goal context
 * Returns bonus score (0-30) based on keyword matches
 */
export function computeKeywordBonus(
  ...
  // Up to 30 bonus points for keyword matches
...
/**
 * Check if XIMAtar fits seniority level
 * Returns true if the seniority is within the profile's range
 */
export function checkSeniorityFit(
  ximatarId: string,
