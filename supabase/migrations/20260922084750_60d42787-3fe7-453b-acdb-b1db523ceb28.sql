...
begin
  ...
  if new.membership_tier is distinct from old.membership_tier
     ...
     or new.subscriber_code is distinct from old.subscriber_code
  ...
  -- Progression / leveling columns: derived server-side only
...
CREATE POLICY "tests: authenticated read" ON public.tests
