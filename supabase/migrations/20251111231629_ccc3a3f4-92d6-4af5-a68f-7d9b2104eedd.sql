-- Update function to show all users with profiles, not just assessed ones
...
AS $$
  SELECT
    ...
    COALESCE(x.image_url, '/ximatars/fox.png') as ximatar_image,
  ...
  FROM profiles p
  LEFT JOIN assessment_results ar ON ar.user_id = p.user_id AND ar.computed_at = (
    SELECT MAX(computed_at) FROM assessment_results WHERE user_id = p.user_id
  )
