-- Update assign_ximatar_by_pillars to add Cat bonus
...
BEGIN
  ...
  ELSE
    ...
    LOOP
      ...
      -- Special bonus for Fox: creativity >= 9 AND communication >= 8.5
      IF ximatar_rec.label = 'fox' AND crea_score >= 9.0 AND comm_score >= 8.5 THEN
        current_distance := current_distance * 0.7;
      END IF;

      -- Special bonus for Dolphin: communication >= 8.5 AND creativity >= 7.5
      IF ximatar_rec.label = 'dolphin' AND comm_score >= 8.5 AND crea_score >= 7.5 THEN
        current_distance := current_distance * 0.7;
      END IF;

      -- Special bonus for Cat: computational >= 8.0 AND creativity >= 8.0 AND communication <= 7.0
