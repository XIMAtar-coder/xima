...
serve(async (req) => {
  ...
  try {
    ...
    const budgetGate = await enforceAiBudget(user.id, "evaluate-growth-test", corsHeaders);
    ...
    if (_ximatarArchetype && _ximatarArchetype.includes("-")) {
      ...
      } catch (e) { console.warn("[evaluate-growth-test] XIMAtar resolution failed:", e); }
    }
...
Return ONLY valid JSON:
{
  "results": {
    ...
    await recordAiCallSafe(user.id, "evaluate-growth-test");
