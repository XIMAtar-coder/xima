/**
 * Server-side copy of the L1 signal heuristic.
 *
 * VERBATIM copy of src/lib/signals/computeSignals.ts. Kept byte-identical below
 * the header so the score a candidate is shown matches the score the business
 * ranks them by; computeSignals.test.ts asserts the two files stay in sync.
 *
 * Why a copy exists at all: this heuristic used to run ONLY in the browser, and
 * the candidate's own client wrote the result into
 * challenge_submissions.signals_payload — the column feeding the 20-point
 * performance axis of the shortlist. Anyone could set their own score to 100 by
 * editing the request. Closing that (the guard trigger added in
 * protect_scoring_columns_from_self_award) broke submission outright, because
 * the app depended on the client being able to write it.
 *
 * The function is pure and deterministic over the candidate's own answers, so
 * the server can recompute it exactly. The candidate still influences the score
 * — by writing a better answer, which is the point — but no longer chooses it.
 */
